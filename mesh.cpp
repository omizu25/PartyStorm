...
CMesh3D::CMesh3D()
{
	...
	m_fVtxHeight = nullptr;								// 頂点の高さ
	m_fAddWave = 0.0f;									// 波の追加分の大きさ
	m_fAmplitude = 0.0f;								// 振幅
	m_bWave = false;									// 波運動するどうか
}
...
{// デバイスのゲット
	...
	if (m_fVtxHeight != nullptr)
	{
		delete[] m_fVtxHeight;
		m_fVtxHeight = nullptr;
	}
	...
	if (m_fVtxHeight == nullptr)
	{
		m_fVtxHeight = new float[m_nVtx];
		assert(m_fVtxHeight != nullptr);
	}
...
// 概要 : 波の設定
//=============================================================================
void CMesh3D::SetWave(const float fAddWave, const float fAmplitude)
{
	// 波の設定
	m_bWave = true;
